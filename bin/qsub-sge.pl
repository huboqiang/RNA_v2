...
Notice: blank line and line start with '#' will not be counted as one line for '--lines'
...
my ($Global,$Queue, $Interval, $Lines, $Maxjob, $Convert,$Secure,$Reqsub,$Resource,$Job_prefix,$Verbose, $Help, $getmem);
GetOptions(
	"global"=>\$Global,
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
...
#zhangjibin
$num_work=$Lines if($num_work>$Lines);
...
while (@Shell) {
	...
	if (-d "/xxxxxx")
	{
		$job_cmd = "qsub -cwd -S /bin/bash ";  ## -l h_vmem=16G,s_core=8
	}
	...
	for (my $i=0; $i<@Shell; $i++) {
		while (1) {
			...
			if ($i < $Maxjob || ($run_num != -1 && $run_num < $Maxjob) ) {
				...
				my $jod_return = `$job_cmd -l $part_resource -V $Shell[$i]`;
	...
	foreach my $job_id (sort keys %Alljob) {
		...
		my $finish_work=`grep -c "This-Work-is-Completed" $shell_file.o$job_id`;
		##check whether the job has been killed during running time
		if ($finish_work < $num_work) {
...
sub run_count {
	...
	return $run_num; ##running jobs in qstat, not zombie ones
}
